...
static Lisp sch_macro_let(Lisp args, LispError* e, LispContext ctx)
{
    ...
    Lisp pairs = lisp_car(args);
    if (lisp_type(pairs) != LISP_PAIR) {
        *e = LISP_ERROR_BAD_LET;
        return lisp_make_null();
    }

    Lisp body = lisp_cdr(args);
    ...
    {
        if (!lisp_is_pair(lisp_car(pairs))) {
            *e = LISP_ERROR_BAD_LET;
            return lisp_make_null();
        }
    ...
    return lisp_cons(lambda, exprs, ctx);
}

static Lisp sch_macro_do(Lisp args, LispError* e, LispContext ctx)
{
    ...
    while (lisp_is_pair(var_list))
    {
        ...
        if (lisp_type(lisp_car(vars)) != LISP_SYMBOL) {
            *e = LISP_ERROR_BAD_DO;
            return lisp_make_null();
        }
    ...
    Lisp loop_test = lisp_car(lisp_list_ref(args, 1));
    Lisp loop_result = lisp_car(lisp_cdr(lisp_list_ref(args, 1)));
    Lisp body = lisp_list_ref(args, 2);
...
static Lisp sch_macro_and(Lisp args, LispError* e, LispContext ctx)
{
    // (AND <pred0> <pred1> ... <predN>) 
    // -> (IF <pred0> 
    //      (IF <pred1> ...
    //          (IF <predN> t f)
    if (lisp_list_length(args) < 1) {
        *e = LISP_ERROR_BAD_AND;
        return lisp_make_null();
    }

    Lisp if_symbol = get_sym(SYM_IF, ctx);

    Lisp preds = lisp_list_reverse(args);
    Lisp p = lisp_car(preds);
...
static Lisp sch_macro_or(Lisp args, LispError* e, LispContext ctx)
{
...
LispContext lisp_init_lib_opt(int symbol_table_size, size_t stack_depth, size_t page_size)
{
    ...
    lisp_table_define_funcs(ctx.impl->macros, lib_cmacro_defs, ctx);
    ...
    LispError error;
    lisp_eval_opt(lisp_read(lib_program_defs, NULL, ctx), system_env, &error, ctx);
